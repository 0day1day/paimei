...
class PAIMEIframe (wx.Frame):
	...
	def __init__ (self, *args, **kwds):
		...
		connect_menu.Append(107, "MySQL &Disconnect","Disconnect from MySQL server.")
		...
		self.Bind(wx.EVT_MENU, self.OnMenuMySQLDisconnect,	id=107)
	...
	def OnMenuMySQLDisconnect (self, event):
		'''
		Event handler for the connections\mysql menu item.
		'''
		if self.mysql is None:
			self.status_bar.SetStatusText("Not connected to MySQL server.")
		else:
			self.mysql.close()
			self.mysql = None
			self.status_bar.SetStatusText("Connection closed.")
