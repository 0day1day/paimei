...
class ReconListCtrl (wx.ListCtrl, ListCtrlAutoWidthMixin, ColumnSorterMixin):
	...
	def on_right_click (self, event):
		...
		if not hasattr(self, "right_click_popup_refresh"):
			...
			self.right_click_popup_delete_all	= wx.NewId()
		...
		menu.Append(self.right_click_popup_delete_all, "Delete &All")
	...
	####################################################################################################################
	def on_right_click_popup_delete_all (self, event):
		'''
		Right click event handler for popup delete all menu selection.
		'''

		recon_id = self.selected_id

		# make sure the user is sure about this action.
		dlg = wx.MessageDialog(self, 'Delete all recon points?', 'Are you sure?', wx.YES_NO | wx.ICON_QUESTION)

		if dlg.ShowModal() != wx.ID_YES:
			return

		cursor = self.top.main_frame.mysql.cursor()
		cursor.execute("DELETE FROM pp_hits WHERE module_id = '%d'" % self.top.module["id"])
		cursor.close()

		# empty the recon list control.
		self.DeleteAllItems()
	####################################################################################################################
